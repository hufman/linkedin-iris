...
class Template(object):
    ...
    def on_get(self, req, resp, template_id):
        ...
        if results:
            ...
            t['content'] = content
            cursor = connection.cursor(db.dict_cursor)
            cursor.execute(text(single_template_query_plans), t['name'])
...
class Application(object):
    ...
    def on_get(self, req, resp, app_name):
        ...
        cursor.execute(text(get_default_application_modes_query), app_name)
        app['default_modes'] = {row['priority']: row['mode'] for row in cursor}

        cursor.execute(text(get_supported_application_modes_query), app['id'])
        app['supported_modes'] = [row['name'] for row in cursor]

        cursor.execute(text(get_application_owners_query), app['id'])
        app['owners'] = [row['name'] for row in cursor]

        cursor.execute(text(get_application_custom_sender_addresses), app['id'])
        app['custom_sender_addresses'] = {row['mode_name']: row['address'] for row in cursor}

        cursor.execute(text(get_application_categories), app['id'])
...
class Applications(object):
    ...
    def on_get(self, req, resp):
        connection = db.engine.raw_connection()
        cursor = connection.cursor(db.dict_cursor)
        cursor.execute(text(get_applications_query + ' ORDER BY `application`.`name` ASC'))
        ...
        for app in apps:
            ...
            cursor.execute(text(get_default_application_modes_query), app['name'])
            app['default_modes'] = {row['priority']: row['mode'] for row in cursor}
...
class ResponseEmail(ResponseMixin):
    def on_post(self, req, resp):
        ...
        if to:
            ...
            with db.guarded_session() as session:
                query = '''SELECT `incident_emails`.`application_id`, `plan_active`.`plan_id`
                           FROM `incident_emails`
                ...
                if email_check_result:
                    ...
                    '''
                    app_template_count = session.execute(text(query), {'app_id': email_check_result['application_id'],
